import secrets
from eb_fast_api.snippets.jwt_service.sources.jwt_service import JWTDecoder, JWTEncoder, JWTService
from eb_fast_api.snippets.jwt_service.testings.mock_jwt_service import MockJWTService, mockNow, mockEmail
...
class TestJWTSerialization:
    ...
    def test_serialize_SUCCESS(self):
        ...
        try:
            ...
            decoded: Optional[dict] = self.decoder.decode(token = token, secretKey = self.secretKey, algorithm=self.algorithm,)
            if decoded == None:
...
class TestJWTService:
    ...
    def test_createAccessToken(self, mockJWTService):
        ...
        expireDate = mockJWTService.dateTimeNow() + timedelta(minutes=mockJWTService.accessTokenExpireMinute)
    ...
    def test_createRefreshToken(self, mockJWTService):
        ...
        expireDate = mockJWTService.dateTimeNow() + timedelta(minutes=mockJWTService.refreshTokenExpireMinute)
        assert result == mockEmail + str(expireDate)
    ...
    def test_checkTokenExpired_NOT_EXPIRE(self, mockJWTService):
        ...
        decoded = mockJWTService.decoder.decode('', '', '')
